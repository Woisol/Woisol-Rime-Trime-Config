...
function translator(input, seg)
		...
		if string.match(input, pattern) ~= nil then
			...
			for cidx, cname in ipairs(colorName) do
				if varW == cname then
					...
					if keyW == "tc" then
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, pattern, "\\textcolor{" .. cname .. "}{}", 1), " "))
					elseif keyW == "box" then
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, pattern, "\\fcolorbox{" .. cname .. "}{white}{}", 1), " "))
					end
				end
			end
		...
		if (string.match(input, pattern) ~= nil) then
			...
			for xlidx, xlNames in ipairs(xlNamesArry) do
				...
				if (string.match(input, pattern .. ".+_") ~= nil) then
					...
					if (string.match(input, thisPattern) ~= nil) then
						...
						for k, name in ipairs(names) do
							for i = 1, 8 do
								yield(Candidate("latex", seg.start, seg._end,
									string.gsub(input, thisPattern, keyW .. "{\\" .. name .. "_{" .. tostring(i) .. "}}",
										1), " "))
							end
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, keyW .. "{\\" .. name .. "_{n}}", 1), " "))
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, keyW .. "{\\" .. name .. "_{0}}", 1), " "))
						end
						--标记过程状态
						matchFlg = true
					end
				elseif (string.match(input, pattern .. ".+^") ~= nil) then
					...
					if (string.match(input, thisPattern) ~= nil) then
						...
						for k, name in ipairs(names) do
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, keyW .. "{\\" .. name .. "}", 1), " "))
							for i = 2, 9 do
								yield(Candidate("latex", seg.start, seg._end,
									string.gsub(input, thisPattern, keyW .. "{\\" .. name .. "^{" .. tostring(i) .. "}}",
										1), " "))
							end
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, keyW .. "{\\" .. name .. "^{n}}", 1), " "))
						end
						--标记过程状态
						matchFlg = true
					end
					...
					if (string.match(input, thisPattern) ~= nil) then
						...
						for k, name in ipairs(names) do
							...
							for i = 2, 9 do
								yield(Candidate("latex", seg.start, seg._end,
									string.gsub(input, thisPattern, keyW .. "^{" .. tostring(i) .. "}{\\" .. name .. "}",
										1), " "))
							end
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, keyW .. "^{n}{\\" .. name .. "}", 1), " "))
						end
						--标记过程状态
						matchFlg = true
					end
			...
			if (string.match(input, thisPattern) ~= nil) then
				...
				if varW_1 == '_' then
					...
					for i = 1, 8 do
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern, keyW .. "{" .. varW .. "_{" .. tostring(i) .. "}}", 1), " "))
					end
					yield(Candidate("latex", seg.start, seg._end,
						string.gsub(input, thisPattern, keyW .. "{" .. varW .. "_{n}}", 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						string.gsub(input, thisPattern, keyW .. "{" .. varW .. "_{0}}", 1), " "))
				...
				elseif varW_1 == '^' then
					--匹配正常字母参数,带上标
					yield(Candidate("latex", seg.start, seg._end,
						string.gsub(input, thisPattern, keyW .. "{" .. varW .. "}", 1), " "))
					for i = 2, 9 do
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern, keyW .. "{" .. varW .. "^{" .. tostring(i) .. "}}", 1), " "))
					end
					yield(Candidate("latex", seg.start, seg._end,
						string.gsub(input, thisPattern, keyW .. "{" .. varW .. "^{n}}", 1), " "))
					--标记过程状态
					matchFlg = true
				else
					--匹配正常字母参数，无上下标
					...
					for i = 2, 9 do
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern, keyW .. "^{" .. tostring(i) .. "}{" .. varW .. "}", 1), " "))
					end
					yield(Candidate("latex", seg.start, seg._end,
						string.gsub(input, thisPattern, keyW .. "^{n}{" .. varW .. "}", 1), " "))
					--标记过程状态
					matchFlg = true
				end
			end
			...
			if (string.match(input, thisPattern) ~= nil) then
				...
				for i = 2, 9 do
					yield(Candidate("latex", seg.start, seg._end,
						string.gsub(input, thisPattern, keyW .. "^{" .. tostring(i) .. "}()", 1), " "))
				end
	...
	for pidx, pattern in ipairs(patterns) do
		if (string.match(input, pattern) ~= nil) then
				...
				if (string.match(input, "d" .. yNameKey .. "d[a-zA-Z]+_?") ~= nil) then
					...
					for xidx, xNames in ipairs(xlNamesArry) do
						...
						if (string.match(input, thisPattern) ~= nil) then
							...
							for fk, fn in ipairs(yNameNames) do
								for xk, xn in ipairs(xNameNames) do
									yield(Candidate("latex", seg.start, seg._end,
										string.gsub(input, thisPattern,
											"\\left.\\cfrac{d\\" .. fn .. "}{d\\" .. xn .. "}\\right|_{\\" .. xn .. "=0}",
											1), " "))
									for i = 2, 9 do
										yield(Candidate("latex", seg.start, seg._end,
											string.gsub(input, thisPattern,
						...
						if (string.match(input, thisPattern) ~= nil) then
							...
							for fk, fn in ipairs(yNameNames) do
								for xk, xn in ipairs(xNameNames) do
									yield(Candidate("latex", seg.start, seg._end,
										string.gsub(input, thisPattern, "\\frac{d\\" .. fn .. "}{d\\" .. xn .. "}", 1),
										" "))
									for i = 2, 9 do
										yield(Candidate("latex", seg.start, seg._end,
											string.gsub(input, thisPattern,
												"\\frac{d^{" ..
												tostring(i) .. "}\\" .. fn .. "}{d\\" .. xn .. "^{" .. tostring(i) ..
												"}}", 1), " "))
									end
									yield(Candidate("latex", seg.start, seg._end,
										string.gsub(input, thisPattern, "\\frac{d^{n}\\" .. fn .. "}{d\\" .. xn ..
											"^{n}}", 1), " "))
								end
							end
							--标记过程状态
					...
					if (string.match(input, thisPattern) ~= nil) then
						...
						for fk, fn in ipairs(yNameNames) do
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern,
									"\\left.\\cfrac{d\\" .. fn .. "}{d" .. varW .. "}\\right|_{" .. varW .. "=0}", 1),
								" "))
							for i = 2, 9 do
								yield(Candidate("latex", seg.start, seg._end,
									string.gsub(input, thisPattern,
										"\\left.\\cfrac{d^{" ..
										tostring(i) ..
										"}\\" .. fn .. "}{d" .. varW .. "^{" .. tostring(i) ..
										"}}\\right|_{" .. varW .. "=0}", 1), " "))
							end
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern,
									"\\left.\\cfrac{d^{n}\\" .. fn .. "}{d" .. varW .. "^{n}}\\right|_{" .. varW .. "=0}",
									1), " "))
						end
						--标记过程状态
						matchFlg = true
					...
					if (string.match(input, thisPattern) ~= nil) then
						...
						for fk, fn in ipairs(yNameNames) do
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, "\\cfrac{d\\" .. fn .. "}{d" .. varW .. "}", 1), " "))
							for i = 2, 9 do
								yield(Candidate("latex", seg.start, seg._end,
									string.gsub(input, thisPattern,
										"\\cfrac{d^{" .. tostring(i) .. "}\\" .. fn ..
										"}{d" .. varW .. "^{" .. tostring(i) .. "}}", 1), " "))
							end
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, "\\cfrac{d^{n}\\" .. fn .. "}{d" .. varW .. "^{n}}", 1),
								" "))
						end
						--标记过程状态
						matchFlg = true
			...
			for xidx, xNames in ipairs(xlNamesArry) do
				...
				if (string.match(input, thisPattern) ~= nil) then
					...
					for xk, xn in ipairs(xNameNames) do
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern,
								"\\left.\\cfrac{d" .. varW_1 .. "}{d\\" .. xn .. "}\\right|_{\\" .. xn .. "=0}", 1), " "))
						for i = 2, 9 do
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern,
									"\\left.\\cfrac{d^{" ..
									tostring(i) ..
									"}" .. varW_1 .. "}{d\\" .. xn .. "^{" .. tostring(i) .. "}}\\right|_{\\" ..
									xn .. "=0}", 1), " "))
						end
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern,
								"\\left.\\cfrac{d^{n}" .. varW_1 .. "}{d\\" .. xn .. "^{n}}\\right|_{\\" .. xn .. "=0}",
								1), " "))
					end
					--标记过程状态
					matchFlg = true
				...
				if (string.match(input, thisPattern) ~= nil) then
					...
					for xk, xn in ipairs(xNameNames) do
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern, "\\cfrac{d" .. varW_1 .. "}{d\\" .. xn .. "}", 1), " "))
						for i = 2, 9 do
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern,
									"\\cfrac{d^{" .. tostring(i) .. "}" .. varW_1 .. "}{d\\" ..
									xn .. "^{" .. tostring(i) .. "}}", 1), " "))
						end
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern, "\\cfrac{d^{n}" .. varW_1 .. "}{d\\" .. xn .. "^{n}}", 1),
							" "))
					end
					--标记过程状态
					matchFlg = true
			...
			if (string.match(input, thisPattern) ~= nil) then
				...
				yield(Candidate("latex", seg.start, seg._end,
					string.gsub(input, thisPattern,
						"\\left.\\cfrac{d" .. varW_1 .. "}{d" .. varW_2 .. "}\\right|_{" .. varW_2 .. "=0}", 1), " "))
				for i = 2, 9 do
					yield(Candidate("latex", seg.start, seg._end,
						string.gsub(input, thisPattern,
							"\\left.\\cfrac{d^{" ..
							tostring(i) .. "}" .. varW_1 ..
							"}{d" .. varW_2 .. "^{" .. tostring(i) .. "}}\\right|_{" .. varW_2 .. "=0}", 1), " "))
				end
				yield(Candidate("latex", seg.start, seg._end,
					string.gsub(input, thisPattern,
						"\\left.\\cfrac{d^{n}" .. varW_1 .. "}{d" .. varW_2 .. "^{n}}\\right|_{" .. varW_2 .. "=0}", 1),
					" "))
			...
			if (string.match(input, thisPattern) ~= nil) then
				...
				yield(Candidate("latex", seg.start, seg._end,
					string.gsub(input, thisPattern, "\\cfrac{d" .. varW_1 .. "}{d" .. varW_2 .. "}", 1), " "))
				for i = 2, 9 do
					yield(Candidate("latex", seg.start, seg._end,
						string.gsub(input, thisPattern,
							"\\cfrac{d^{" .. tostring(i) .. "}" .. varW_1 .. "}{d" .. varW_2 .. "^{" .. tostring(i) ..
							"}}", 1), " "))
				end
				yield(Candidate("latex", seg.start, seg._end,
					string.gsub(input, thisPattern, "\\cfrac{d^{n}" .. varW_1 .. "}{d" .. varW_2 .. "^{n}}", 1), " "))
	...
	for pos = 1, string.len(input) do
		...
		for idx, pattern in ipairs(patterns) do
			if string.match(input, pattern, pos) ~= nil then
				...
				if str == "alig" then
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\begin{aligned}\r  \\end{aligned}", 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 ..
						string.gsub(subStr_1, pattern,
							"\\begin{equation}\\begin{aligned}\r    \\end{aligned}\\end{equation}", 1), " "))
					matchFlg = true
				elseif str == "appr" then
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\approx ", 1), " "))
					matchFlg = true
				elseif str == "arra" then
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 ..
						string.gsub(subStr_1, pattern, "\\begin{array}{rl} ab & c \\\\ b & cd \\end{array}", 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 ..
						string.gsub(subStr_1, pattern, "\\begin{array}{cc} ab & c \\\\ b & cd \\end{array}", 1), " "))
					matchFlg = true
				elseif str == "beca" then
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\because ", 1), " "))
					matchFlg = true
				elseif str == "canc" then
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\cancel{}", 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\xcancel{}", 1), " "))
					matchFlg = true
				elseif str == "case" then
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\begin{cases}\r ?&=? \\\\ \r\\end{cases}", 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 ..
						string.gsub(subStr_1, pattern, "x=\\begin{cases}\r ?=? &\\text{if} ? \\\\ \r\\end{cases}", 1),
						" "))
					matchFlg = true
				elseif str == "disp" then
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\displaystyle", 1), " "))
					matchFlg = true
				elseif str == "exis" then
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\exist ", 1), " "))
					matchFlg = true
				elseif str == "fora" then
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\forall ", 1), " "))
					matchFlg = true
				elseif str == "frac" then
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\frac{}{}", 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\cfrac{}{}", 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\dfrac{}{}", 1), " "))
					matchFlg = true
				elseif str == "grou" then
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\overgroup{}", 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\undergroup{}", 1), " "))
					matchFlg = true
				elseif str == "idxx" then
					for i = 1, 10 do
						yield(Candidate("latex", seg.start, seg._end,
							subStr_0 .. string.gsub(subStr_1, pattern, "式(" .. tostring(i) .. ")", 1), " "))
					end
					matchFlg = true
				elseif str == "infi" then
					yield(Candidate("latex", seg.start, seg._end, subStr_0 ..
						string.gsub(subStr_1, pattern, "\\infin", 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "-\\infin", 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "+\\infin", 1), " "))
					matchFlg = true
				elseif str == "line" then
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\overline{}", 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\underline{}", 1), " "))
					matchFlg = true
				elseif str == "matr" then
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 ..
						string.gsub(subStr_1, pattern, "\\begin{matrix}\r a & b \\\\\r c & d \r\\end{matrix}", 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 ..
						string.gsub(subStr_1, pattern, "\\begin{bmatrix}\r a & b \\\\\r c & d \r\\end{bmatrix}", 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 ..
						string.gsub(subStr_1, pattern, "\\begin{pmatrix}\r a & b \\\\\r c & d \r\\end{pmatrix}", 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 ..
						string.gsub(subStr_1, pattern, "\\begin{vmatrix}\r a & b \\\\\r c & d \r\\end{vmatrix}", 1), " "))
					matchFlg = true
				elseif str == "nexi" then
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\nexists ", 1), " "))
					matchFlg = true
				elseif str == "prod" then
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\prod_{}^{n}{}", 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\displaystyle\\prod_{}^{n}{}", 1), " "))
					matchFlg = true
				elseif str == "suba" then
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\begin{subarray}{c} ab \\\\ c \\end{subarray}", 1),
						" "))
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\begin{subarray}{l} ab \\\\ c \\end{subarray}", 1),
						" "))
					matchFlg = true
				elseif str == "subs" then
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\substack{ab\\\\c}", 1), " "))
					matchFlg = true
				elseif str == "sout" then
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\sout{}", 1), " "))
					matchFlg = true
				elseif str == "tria" then
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\triangleq ", 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\vartriangle ", 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\vartriangleleft ", 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\vartriangleright ", 1), " "))
					matchFlg = true
				elseif str == "ther" then
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\therefore ", 1), " "))
					matchFlg = true
				end
			end
			if matchFlg then
	...
	for pidx, pattern in ipairs(patterns) do
		if (string.match(input, pattern) ~= nil) then
				...
				if (string.match(input, "log" .. bNameKey .. "[a-zA-Z]*_?") ~= nil) then
					...
					for xidx, xNames in ipairs(xlNamesArry) do
							...
							for bk, bn in ipairs(bNameNames) do
								for xk, xn in ipairs(xNameNames) do
									for i = 1, 8 do
										yield(Candidate("latex", seg.start, seg._end,
											string.gsub(input, thisPattern,
												"\\log_{\\" .. bn .. "}\\" .. xn .. "_{" .. tostring(i) .. "}", 1), " "))
									end
									yield(Candidate("latex", seg.start, seg._end,
										string.gsub(input, thisPattern, "\\log_{\\" .. bn .. "}\\" .. xn .. "_{n}", 1),
										" "))
									yield(Candidate("latex", seg.start, seg._end,
										string.gsub(input, thisPattern, "\\log_{\\" .. bn .. "}\\" .. xn .. "_{0}", 1),
										" "))
								end
							end
							--标记过程状态
						...
						if (string.match(input, thisPattern) ~= nil) then
							...
							for bk, bn in ipairs(bNameNames) do
								for xk, xn in ipairs(xNameNames) do
									yield(Candidate("latex", seg.start, seg._end,
										string.gsub(input, thisPattern, "\\log_{\\" .. bn .. "}\\" .. xn .. " ", 1), " "))
									for i = 2, 9 do
										yield(Candidate("latex", seg.start, seg._end,
											string.gsub(input, thisPattern,
												"\\log_{\\" .. bn .. "}\\" .. xn .. "^{" .. tostring(i) .. "}", 1), " "))
									end
									yield(Candidate("latex", seg.start, seg._end,
										string.gsub(input, thisPattern, "\\log_{\\" .. bn .. "}\\" .. xn .. "^{n}", 1),
										" "))
								end
							end
							--标记过程状态
							...
							for bk, bn in ipairs(bNameNames) do
								for xk, xn in ipairs(xNameNames) do
									...
									for i = 2, 9 do
										yield(Candidate("latex", seg.start, seg._end,
											string.gsub(input, thisPattern,
												"\\log_{\\" .. bn .. "}^{" .. tostring(i) .. "}\\" .. xn .. " ", 1), " "))
									end
									yield(Candidate("latex", seg.start, seg._end,
										string.gsub(input, thisPattern, "\\log_{\\" .. bn .. "}^{n}\\" .. xn .. " ", 1),
										" "))
								end
							end
							--标记过程状态
					...
					if (string.match(input, thisPattern) ~= nil) then
						...
						for bk, bn in ipairs(bNameNames) do
							for i = 1, 8 do
								yield(Candidate("latex", seg.start, seg._end,
									string.gsub(input, thisPattern, "\\log_{\\" .. bn ..
										"}" .. varW .. "_{" .. tostring(i) .. "}", 1), " "))
							end
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, "\\log_{\\" .. bn .. "}" .. varW .. "_{n}", 1), " "))
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, "\\log_{\\" .. bn .. "}" .. varW .. "_{0}", 1), " "))
						end
						--标记过程状态
						matchFlg = true
					end
					...
					if (string.match(input, thisPattern) ~= nil) then
						...
						for bk, bn in ipairs(bNameNames) do
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, "\\log_{\\" .. bn .. "}" .. varW .. " ", 1), " "))
							for i = 2, 9 do
								yield(Candidate("latex", seg.start, seg._end,
									string.gsub(input, thisPattern, "\\log_{\\" .. bn ..
										"}" .. varW .. "^{" .. tostring(i) .. "}", 1), " "))
							...
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, "\\log_{\\" .. bn .. "}" .. varW .. "^{n}", 1), " "))
						end
						--标记过程状态
						matchFlg = true
					end
					...
					if (string.match(input, thisPattern) ~= nil) then
						...
						for bk, bn in ipairs(bNameNames) do
							...
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, "\\log_{\\" .. bn .. "}^{n}" .. varW .. " ", 1), " "))
						end
						--标记过程状态
						matchFlg = true
					end
					...
					if (string.match(input, thisPattern) ~= nil) then
						...
						for bk, bn in ipairs(bNameNames) do
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, "\\log_{\\" .. bn .. "}()", 1), " "))
							for i = 2, 9 do
								yield(Candidate("latex", seg.start, seg._end,
									string.gsub(input, thisPattern, "\\log_{\\" .. bn .. "}^{" .. tostring(i) .. "}()", 1),
									" "))
							end
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, "\\log_{\\" .. bn .. "}^{n}()", 1), " "))
						end
						--标记过程状态
						matchFlg = true
					end
			...
			for xidx, xNames in ipairs(xlNamesArry) do
				...
				if (string.match(input, thisPattern) ~= nil) then
					...
					for xk, xn in ipairs(xNameNames) do
						for i = 1, 8 do
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, "\\log_{" .. varW_1 .. "}\\" ..
									xn .. "_{" .. tostring(i) .. "}", 1), " "))
						end
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern, "\\log_{" .. varW_1 .. "}\\" .. xn .. "_{n}", 1), " "))
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern, "\\log_{" .. varW_1 .. "}\\" .. xn .. "_{0}", 1), " "))
					end
				...
				if (string.match(input, thisPattern) ~= nil) then
					...
					for xk, xn in ipairs(xNameNames) do
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern, "\\log_{" .. varW_1 .. "}\\" .. xn .. " ", 1), " "))
						for i = 2, 9 do
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, "\\log_{" .. varW_1 .. "}\\" ..
									xn .. "^{" .. tostring(i) .. "}", 1), " "))
						...
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern, "\\log_{" .. varW_1 .. "}\\" .. xn .. "^{n}", 1), " "))
					end
				...
				if (string.match(input, thisPattern) ~= nil) then
					...
					for xk, xn in ipairs(xNameNames) do
						...
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern, "\\log_{" .. varW_1 .. "}^{n}\\" .. xn .. " ", 1), " "))
					end
			...
			if (string.match(input, thisPattern) ~= nil) then
				...
				for i = 1, 8 do
					yield(Candidate("latex", seg.start, seg._end,
						string.gsub(input, thisPattern, "\\log_{" .. varW_1 .. "}" .. varW_2 .. "_{" .. tostring(i) ..
							"}", 1), " "))
				end
				yield(Candidate("latex", seg.start, seg._end,
					string.gsub(input, thisPattern, "\\log_{" .. varW_1 .. "}" .. varW_2 .. "_{n}", 1), " "))
				yield(Candidate("latex", seg.start, seg._end,
					string.gsub(input, thisPattern, "\\log_{" .. varW_1 .. "}" .. varW_2 .. "_{0}", 1), " "))
			...
			if (string.match(input, thisPattern) ~= nil) then
				...
				yield(Candidate("latex", seg.start, seg._end,
					string.gsub(input, thisPattern, "\\log_{" .. varW_1 .. "}" .. varW_2 .. " ", 1), " "))
				for i = 2, 9 do
					yield(Candidate("latex", seg.start, seg._end,
						string.gsub(input, thisPattern, "\\log_{" .. varW_1 .. "}" .. varW_2 .. "^{" .. tostring(i) ..
							"}", 1), " "))
				end
				yield(Candidate("latex", seg.start, seg._end,
					string.gsub(input, thisPattern, "\\log_{" .. varW_1 .. "}" .. varW_2 .. "^{n}", 1), " "))
			...
			if (string.match(input, thisPattern) ~= nil) then
				...
				for i = 2, 9 do
					yield(Candidate("latex", seg.start, seg._end,
						string.gsub(input, thisPattern, "\\log_{" .. varW_1 .. "}^{" .. tostring(i) .. "}" .. varW_2 ..
							" ", 1), " "))
				end
				yield(Candidate("latex", seg.start, seg._end,
					string.gsub(input, thisPattern, "\\log_{" .. varW_1 .. "}^{n}" .. varW_2 .. " ", 1), " "))
			...
			if (string.match(input, thisPattern) ~= nil) then
				...
				yield(Candidate("latex", seg.start, seg._end,
					string.gsub(input, thisPattern, "\\log_{" .. varW_1 .. "}()", 1), " "))
				for i = 2, 9 do
					yield(Candidate("latex", seg.start, seg._end,
						string.gsub(input, thisPattern, "\\log_{" .. varW_1 .. "}^{" .. tostring(i) .. "}()", 1), " "))
				end
				yield(Candidate("latex", seg.start, seg._end,
					string.gsub(input, thisPattern, "\\log_{" .. varW_1 .. "}^{n}()", 1), " "))
			...
			if (string.match(input, thisPattern) ~= nil) then
				...
				for i = 2, 9 do
					yield(Candidate("latex", seg.start, seg._end,
						string.gsub(input, thisPattern, "\\log_{?}^{" .. tostring(i) .. "}()", 1), " "))
				end
			...
			if keyW == "lim" then
				...
				for xlidx, xlNames in ipairs(xlNamesArry) do
					...
					if (string.match(input, pattern .. ".+_") ~= nil) then
						...
						if (string.match(input, thisPattern) ~= nil) then
							...
							for k, name in ipairs(names) do
								yield(Candidate("latex", seg.start, seg._end,
									string.gsub(input, thisPattern, "\\displaystyle\\" .. keyW ..
										"_{\\" .. name .. "\\to0}{}", 1), " "))
								for i = 2, 9 do
									yield(Candidate("latex", seg.start, seg._end,
										string.gsub(input, thisPattern,
											"\\displaystyle\\" .. keyW ..
											"_{\\" .. name .. "_{" .. tostring(i) .. "}" .. "\\to0}{}", 1), " "))
								end
								yield(Candidate("latex", seg.start, seg._end,
									string.gsub(input, thisPattern,
										"\\displaystyle\\" .. keyW .. "_{\\" .. name .. "_{n}\\to0}{}", 1), " "))

								yield(Candidate("latex", seg.start, seg._end,
									string.gsub(input, thisPattern,
										"\\displaystyle\\" .. keyW .. "_{\\" .. name .. "\\to\\infin}{}", 1), " "))
								for i = 2, 9 do
									yield(Candidate("latex", seg.start, seg._end,
										string.gsub(input, thisPattern,
											"\\displaystyle\\" ..
											keyW .. "_{\\" .. name .. "_{" .. tostring(i) .. "}" .. "\\to\\infin}{}", 1),
										" "))
								end
								yield(Candidate("latex", seg.start, seg._end,
									string.gsub(input, thisPattern,
										"\\displaystyle\\" .. keyW .. "_{\\" .. name .. "_{n}\\to\\infin}{}", 1), " "))
							...
							for k, name in ipairs(names) do
								...
								for i = 2, 9 do
									yield(Candidate("latex", seg.start, seg._end,
										string.gsub(input, thisPattern,
											"\\displaystyle\\" .. keyW ..
											"_{\\" .. name .. "^{" .. tostring(i) .. "}" .. "\\to0}{}", 1), " "))
								end
								yield(Candidate("latex", seg.start, seg._end,
									string.gsub(input, thisPattern,
										"\\displaystyle\\" .. keyW .. "_{\\" .. name .. "^{n}\\to0}{}", 1), " "))
								...
								for i = 2, 9 do
									yield(Candidate("latex", seg.start, seg._end,
										string.gsub(input, thisPattern,
											"\\displaystyle\\" ..
											keyW .. "_{\\" .. name .. "^{" .. tostring(i) .. "}" .. "\\to\\infin}{}", 1),
										" "))
								end
								yield(Candidate("latex", seg.start, seg._end,
									string.gsub(input, thisPattern,
										"\\displaystyle\\" .. keyW .. "_{\\" .. name .. "^{n}\\to\\infin}{}", 1), " "))
				...
				if (string.match(input, thisPattern) ~= nil) then
					...
					if varW_1 == '_' then
						--匹配正常字母参数，带下标
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern, "\\displaystyle\\" .. keyW .. "_{" .. varW .. "\\to0}{}", 1),
							" "))
						for i = 2, 9 do
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern,
									"\\displaystyle\\" .. keyW .. "_{" .. varW .. "_{" .. tostring(i) .. "}" ..
									"\\to0}{}", 1), " "))
						end
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern, "\\displaystyle\\" .. keyW .. "_{" .. varW .. "_{n}\\to0}{}",
								1), " "))

						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern, "\\displaystyle\\" .. keyW .. "_{" .. varW ..
								"\\to\\infin}{}", 1), " "))
						for i = 2, 9 do
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern,
									"\\displaystyle\\" .. keyW .. "_{" .. varW ..
									"_{" .. tostring(i) .. "}" .. "\\to\\infin}{}", 1), " "))
						end
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern, "\\displaystyle\\" .. keyW ..
								"_{" .. varW .. "_{n}\\to\\infin}{}", 1), " "))
					...
					elseif varW_1 == '^' then
						...
						for i = 2, 9 do
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern,
									"\\displaystyle\\" .. keyW .. "_{" .. varW .. "^{" .. tostring(i) .. "}" ..
									"\\to0}{}", 1), " "))
						end
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern, "\\displaystyle\\" .. keyW .. "_{" .. varW .. "^{n}\\to0}{}",
								1), " "))
		...
		if (string.match(input, pattern) ~= nil) then
			...
			for xlidx, xlNames in ipairs(xlNamesArry) do
				...
				if (string.match(input, pattern .. ".+_") ~= nil) then
					...
					if (string.match(input, thisPattern) ~= nil) then
						...
						for k, name in ipairs(names) do
							for i = 1, 8 do
								yield(Candidate("latex", seg.start, seg._end,
									string.gsub(input, thisPattern, keyW .. "{\\" ..
										name .. "_{" .. tostring(i) .. "}}" .. keyWEnd, 1), " "))
							end
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, keyW .. "{\\" .. name .. "_{n}}" .. keyWEnd, 1), " "))
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, keyW .. "{\\" .. name .. "_{0}}" .. keyWEnd, 1), " "))
						end
						--标记过程状态
						matchFlg = true
					end
				elseif (string.match(input, pattern .. ".+^") ~= nil) then
					...
					if (string.match(input, thisPattern) ~= nil) then
						...
						for k, name in ipairs(names) do
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, keyW .. "{\\" .. name .. "}" .. keyWEnd, 1), " "))
							for i = 2, 9 do
								yield(Candidate("latex", seg.start, seg._end,
									string.gsub(input, thisPattern,
										keyW .. "{\\" .. name .. "}" .. keyWEnd .. "^{" .. tostring(i) .. "}", 1), " "))
							end
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, keyW .. "{\\" .. name .. "}" .. keyWEnd .. "^{n}", 1),
								" "))
						end
						--标记过程状态
						matchFlg = true
			...
			if (string.match(input, thisPattern) ~= nil) then
				...
				if varW_1 == '_' then
					...
					for i = 1, 8 do
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern, keyW .. "{" .. varW .. "_{" .. tostring(i) .. "}}" .. keyWEnd,
								1), " "))
					end
					yield(Candidate("latex", seg.start, seg._end,
						string.gsub(input, thisPattern, keyW .. "{" .. varW .. "_{n}}" .. keyWEnd, 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						string.gsub(input, thisPattern, keyW .. "{" .. varW .. "_{0}}" .. keyWEnd, 1), " "))
				...
				elseif varW_1 == '^' then
					--匹配正常字母参数，带上标
					yield(Candidate("latex", seg.start, seg._end,
						string.gsub(input, thisPattern, keyW .. "{" .. varW .. "}" .. keyWEnd, 1), " "))
					for i = 2, 9 do
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern, keyW .. "{" .. varW .. "^{" .. tostring(i) .. "}}" .. keyWEnd,
								1), " "))
					end
					yield(Candidate("latex", seg.start, seg._end,
						string.gsub(input, thisPattern, keyW .. "{" .. varW .. "^{n}}" .. keyWEnd, 1), " "))
					...
					for i = 2, 9 do
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern, keyW .. "{" .. varW .. "}" .. keyWEnd ..
								"^{" .. tostring(i) .. "}", 1), " "))
					end
					yield(Candidate("latex", seg.start, seg._end,
						string.gsub(input, thisPattern, keyW .. "{" .. varW .. "}" .. keyWEnd .. "^{n}", 1), " "))
			...
			if (string.match(input, thisPattern) ~= nil) then
				...
				yield(Candidate("latex", seg.start, seg._end,
					string.gsub(input, thisPattern, keyW .. "{?}" .. keyWEnd, 1),
					" "))
				for i = 2, 9 do
					yield(Candidate("latex", seg.start, seg._end,
						string.gsub(input, thisPattern, keyW .. "{?}" .. keyWEnd .. "^{" .. tostring(i) .. "}", 1), " "))
				end
				yield(Candidate("latex", seg.start, seg._end,
					string.gsub(input, thisPattern, keyW .. "{?}" .. keyWEnd .. "^{n}", 1), " "))
		...
		if (string.match(input, pattern) ~= nil) then
				...
				if (string.match(input, pattern .. ".+_") ~= nil) then
					...
					if (string.match(input, thisPattern) ~= nil) then
						...
						for k, name in ipairs(names) do
							for i = 1, 8 do
								yield(Candidate("latex", seg.start, seg._end,
									string.gsub(input, thisPattern, keyW .. "d\\" .. name .. "_{" .. tostring(i) .. "}",
										1), " "))
							end
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, keyW .. "d\\" .. name .. "_{n}", 1), " "))
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, keyW .. "d\\" .. name .. "_{0}", 1), " "))
						end
						--标记过程状态
						matchFlg = true
					end
					...
					if (string.match(input, thisPattern) ~= nil) then
						...
						for k, name in ipairs(names) do
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, keyW .. "d\\" .. name .. " ", 1), " "))
							for i = 2, 9 do
								yield(Candidate("latex", seg.start, seg._end,
									string.gsub(input, thisPattern, keyW .. "d(\\" .. name .. "^{" .. tostring(i) .. "})",
										1), " "))
							end
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, keyW .. "d(\\" .. name .. "^{n})", 1), " "))
						end
						--标记过程状态
						matchFlg = true
					end
			...
			if (string.match(input, thisPattern) ~= nil) then
				...
				if varW_1 == '_' then
					...
					for i = 1, 8 do
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern, keyW .. "d" .. varW .. "_{" .. tostring(i) .. "}", 1), " "))
					end
					yield(Candidate("latex", seg.start, seg._end,
						string.gsub(input, thisPattern, keyW .. "d" .. varW .. "_{n}", 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						string.gsub(input, thisPattern, keyW .. "d" .. varW .. "_{0}", 1), " "))

					--标记过程状态
					matchFlg = true
				else
					--匹配正常字母参数，不带上下标
					yield(Candidate("latex", seg.start, seg._end, string.gsub(input, thisPattern, keyW .. "d" .. varW ..
						"", 1), " "))
					for i = 2, 9 do
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern, keyW .. "d(" .. varW .. "^{" .. tostring(i) .. "})", 1), " "))
					end
					yield(Candidate("latex", seg.start, seg._end,
						string.gsub(input, thisPattern, keyW .. "d(" .. varW .. "^{n})", 1), " "))
			...
			if (string.match(input, thisPattern) ~= nil) then
				...
				for i = 2, 9 do
					yield(Candidate("latex", seg.start, seg._end,
						string.gsub(input, thisPattern, keyW .. "d(x^{" .. tostring(i) .. "})", 1), " "))
				end
	...
	for idx, pattern in ipairs(patterns) do
		if string.match(input, pattern) ~= nil then
			...
			yield(Candidate("latex", seg.start, seg._end,
				string.gsub(input, pattern, "\\mathring{" .. keyW_u .. "}(" .. varW .. ")", 1), " "))
			yield(Candidate("latex", seg.start, seg._end,
				string.gsub(input, pattern, "\\mathring{" .. keyW_u .. "}(" .. varW .. "_0)", 1), " "))
			yield(Candidate("latex", seg.start, seg._end,
				string.gsub(input, pattern, "\\mathring{" .. keyW_u .. "}(" .. varW .. "_0, \\delta)", 1), " "))
	...
	for idx, pattern in ipairs(patterns) do
		if string.match(input, pattern) ~= nil then
			...
			yield(Candidate("latex", seg.start, seg._end,
				string.gsub(input, pattern, string.upper(keyW) .. "_" .. varW_1 .. "(" .. varW_2 .. ")", 1), " "))
	...
	for pos = 1, string.len(input) do
		...
		for idx, pattern in ipairs(patterns) do
			if string.match(input, pattern, pos) ~= nil then
				...
				if str == "bar" then
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\bar{} ", 1), " "))
					matchFlg = true
				elseif str == "big" then
					yield(Candidate("latex", seg.start, seg._end, subStr_0 ..
						string.gsub(subStr_1, pattern, "\\big ", 1), " "))
					yield(Candidate("latex", seg.start, seg._end, subStr_0 ..
						string.gsub(subStr_1, pattern, "\\Big ", 1), " "))
					yield(Candidate("latex", seg.start, seg._end, subStr_0 ..
						string.gsub(subStr_1, pattern, "\\bigg ", 1), " "))
					yield(Candidate("latex", seg.start, seg._end, subStr_0 ..
						string.gsub(subStr_1, pattern, "\\Bigg ", 1), " "))
					matchFlg = true
				elseif str == "box" then
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\boxed{}", 1), " "))
					matchFlg = true
				elseif str == "cap" then
					yield(Candidate("latex", seg.start, seg._end, subStr_0 ..
						string.gsub(subStr_1, pattern, "\\cap ", 1), " "))
					matchFlg = true
				elseif str == "cup" then
					yield(Candidate("latex", seg.start, seg._end, subStr_0 ..
						string.gsub(subStr_1, pattern, "\\cup ", 1), " "))
					matchFlg = true
				elseif str == "idx" then
					for i = 1, 10 do
						yield(Candidate("latex", seg.start, seg._end,
							subStr_0 .. string.gsub(subStr_1, pattern, "\\quad(" .. tostring(i) .. ")", 1), " "))
					end
					matchFlg = true
				elseif str == "max" then
					yield(Candidate("latex", seg.start, seg._end, subStr_0 ..
						string.gsub(subStr_1, pattern, "\\max()", 1), " "))
					matchFlg = true
				elseif str == "min" then
					yield(Candidate("latex", seg.start, seg._end, subStr_0 ..
						string.gsub(subStr_1, pattern, "\\min()", 1), " "))
					matchFlg = true
				elseif str == "neq" then
					yield(Candidate("latex", seg.start, seg._end, subStr_0 ..
						string.gsub(subStr_1, pattern, "\\neq ", 1), " "))
				...
				elseif str == "set" then
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\subset ", 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\supset ", 1), " "))
					matchFlg = true
				elseif str == "sim" then
					yield(Candidate("latex", seg.start, seg._end, subStr_0 ..
						string.gsub(subStr_1, pattern, "\\sim ", 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\thicksim ", 1), " "))
					matchFlg = true
				elseif str == "sum" then
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\sum_{k=1}^{n}{}", 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\displaystyle\\sum_{k=1}^{n}{}", 1), " "))
					matchFlg = true
				end
			end
			if matchFlg then
			...
			if string.match(input, pattern, pos) ~= nil then
				...
				if str == 'in' then
					...
					yield(Candidate("latex", seg.start, seg._end, subStr_0 .. string.gsub(subStr_1, pattern, "\\in()", 1),
						" "))
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, keyW .. "_{" .. varW .. "}", 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, keyW .. "^{" .. varW .. "}", 1), " "))
					matchFlg = true
				elseif str == 'mp' or str == 'pm' then
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, "\\" .. str .. " ", 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, keyW .. "_{" .. varW .. "}", 1), " "))
					yield(Candidate("latex", seg.start, seg._end,
						subStr_0 .. string.gsub(subStr_1, pattern, keyW .. "^{" .. varW .. "}", 1), " "))
				...
				elseif keyW == 'y' then
					...
					elseif varW == 'x' then
						yield(Candidate("latex", seg.start, seg._end,
							subStr_0 .. string.gsub(subStr_1, pattern, keyW .. "'\\left|_{" .. varW .. "=0}\\right.=", 1),
							" "))
						yield(Candidate("latex", seg.start, seg._end,
							subStr_0 ..
							string.gsub(subStr_1, pattern, keyW .. "''\\left|_{" .. varW .. "=0}\\right.=", 1),
							" "))
						yield(Candidate("latex", seg.start, seg._end,
							subStr_0 .. string.gsub(subStr_1, pattern, keyW .. "'''\\left|_{" .. varW .. "=0}\\right.=",
								1), " "))
						yield(Candidate("latex", seg.start, seg._end,
							subStr_0 .. string.gsub(subStr_1, pattern, keyW .. "''''\\left|_{" .. varW .. "=0}\\right.=",
								1), " "))
						for i = 5, 8 do
							yield(Candidate("latex", seg.start, seg._end,
								subStr_0 ..
								string.gsub(subStr_1, pattern,
									keyW .. "^{(" .. tostring(i) .. ")}\\left|_{" .. varW .. "=0}\\right.=", 1), " "))
						end
						yield(Candidate("latex", seg.start, seg._end,
							subStr_0 .. string.gsub(subStr_1, pattern, keyW .. "^{(n)}\\left|_{" .. varW ..
								"=0}\\right.=", 1), " "))
						yield(Candidate("latex", seg.start, seg._end,
							subStr_0 .. string.gsub(subStr_1, pattern, keyW .. "\\left|_{" .. varW .. "=0}\\right.=", 1),
							" "))
				...
				if (string.match(input, yNameKey .. "[a-zA-Z]+_?") ~= nil) then
					...
					for xidx, xNames in ipairs(xlNamesArry) do
							...
							for fk, fn in ipairs(yNameNames) do
								for xk, xn in ipairs(xNameNames) do
									for i = 1, 8 do
										yield(Candidate("latex", seg.start, seg._end,
											string.gsub(input, thisPattern, "\\" ..
												fn .. "(\\" .. xn .. "_{" .. tostring(i) .. "})", 1), " "))
									end
									yield(Candidate("latex", seg.start, seg._end,
										string.gsub(input, thisPattern, "\\" .. fn .. "(\\" .. xn .. "_{n})", 1), " "))
									yield(Candidate("latex", seg.start, seg._end,
										string.gsub(input, thisPattern, "\\" .. fn .. "(\\" .. xn .. "_{0})", 1), " "))
								end
							end
							--标记过程状态
							matchFlg = true
						...
						if (string.match(input, thisPattern) ~= nil) then
							...
							for fk, fn in ipairs(yNameNames) do
								for xk, xn in ipairs(xNameNames) do
									yield(Candidate("latex", seg.start, seg._end,
										string.gsub(input, thisPattern, "\\" .. fn .. "'(\\" .. xn .. ")", 1), " "))
									yield(Candidate("latex", seg.start, seg._end,
										string.gsub(input, thisPattern, "\\" .. fn .. "''(\\" .. xn .. ")", 1), " "))
									for i = 3, 9 do
										yield(Candidate("latex", seg.start, seg._end,
					...
					if (string.match(input, thisPattern) ~= nil) then
						...
						for fk, fn in ipairs(yNameNames) do
							for i = 1, 8 do
								yield(Candidate("latex", seg.start, seg._end,
									string.gsub(input, thisPattern, "\\" .. fn .. "(" .. varW .. "_{" ..
										tostring(i) .. "})", 1), " "))
							end
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, "\\" .. fn .. "(" .. varW .. "_{n})", 1), " "))
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, "\\" .. fn .. "(" .. varW .. "_{0})", 1), " "))
						end
						--标记过程状态
						matchFlg = true
					end
					...
					if (string.match(input, thisPattern) ~= nil) then
						...
						for fk, fn in ipairs(yNameNames) do
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, "\\" .. fn .. "'(" .. varW .. ")", 1), " "))
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, "\\" .. fn .. "''(" .. varW .. ")", 1), " "))
							for i = 3, 9 do
								yield(Candidate("latex", seg.start, seg._end,
									string.gsub(input, thisPattern, "\\" .. fn .. "^{(" .. tostring(i) ..
										")}(" .. varW .. ")", 1), " "))
							end
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, "\\" .. fn .. "(" .. varW .. ")", 1), " "))
			...
			for xidx, xNames in ipairs(xlNamesArry) do
				...
				if (string.match(input, thisPattern) ~= nil) then
					...
					for xk, xn in ipairs(xNameNames) do
						for i = 1, 8 do
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, keyW .. "(\\" .. xn .. "_{" .. tostring(i) .. "})", 1),
								" "))
						end
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern, keyW .. "(\\" .. xn .. "_{n})", 1), " "))
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern, keyW .. "(\\" .. xn .. "_{0})", 1), " "))
					end
				...
				if (string.match(input, thisPattern) ~= nil) then
					...
					for xk, xn in ipairs(xNameNames) do
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern, keyW .. "'(\\" .. xn .. ")", 1), " "))
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern, keyW .. "''(\\" .. xn .. ")", 1), " "))
						for i = 3, 9 do
							yield(Candidate("latex", seg.start, seg._end,
								string.gsub(input, thisPattern, keyW .. "^{(" .. tostring(i) .. ")}(\\" .. xn .. ")", 1),
								" "))
						end
						yield(Candidate("latex", seg.start, seg._end,
							string.gsub(input, thisPattern, keyW .. "(\\" .. xn .. ")", 1), " "))
	...
	for xlkey, xlNames in ipairs(xlNamesArry) do
		...
		if string.match(input, thisPattern) ~= nil then
			...
			for k, name in ipairs(names) do
				for i = 1, 8 do
					yield(Candidate("latex", seg.start, seg._end,
						string.gsub(input, thisPattern, "\\" .. name .. "_{" .. tostring(i) .. "}", 1), " "))
				end
		...
		if string.match(input, thisPattern) ~= nil then
			...
			for k, name in ipairs(names) do
				...
				for i = 2, 9 do
					yield(Candidate("latex", seg.start, seg._end,
						string.gsub(input, thisPattern, "\\" .. name .. "^{" .. tostring(i) .. "}", 1), " "))
				end
	...
	--@section-符号字符关键字，符号 ( ) . % + - * ?[ ] ^ $ 在lua正则表达式中识别为特殊符号，可以通过 % 转义为符号本身
	patterns = { "^[|%^%+%-_=%*%?%(%)%[%]{}<!@#%$&~']" }
	for idx, pattern in ipairs(patterns) do
		if string.match(input, pattern) ~= nil then
			str = input
			...
			if debugFlg then
				yield(Candidate("latex", seg.start, seg._end, "latexLetters: " .. string.sub(pattern, 1, 10) .. ", " ..
					str, " "))
			end
			...
			if string.len(keyW) == 1 then
				...
				elseif keyW == '|' then
					yield(Candidate("latex", seg.start, seg._end,
						"\\begin{vmatrix}\r a & b \\\\\r c & d \r\\end{vmatrix}", " "))
					yield(Candidate("latex", seg.start, seg._end,
						"\\begin{Vmatrix}\r a & b \\\\\r c & d \r\\end{Vmatrix}", " "))
					matchFlg = true
				elseif keyW == '(' then
					...
					yield(Candidate("latex", seg.start, seg._end,
						"\\begin{pmatrix}\r a & b \\\\\r c & d \r\\end{pmatrix}", " "))
					matchFlg = true
				elseif keyW == '<' then
					for i = 2, 9 do
						yield(Candidate("latex", seg.start, seg._end,
							"\\left\\langle {} \\right\\rangle^{" .. tostring(i) .. "}", " "))
					end
				...
				elseif keyW == '[' then
					...
					yield(Candidate("latex", seg.start, seg._end, "\\left[ \\right]", " "))
					yield(Candidate("latex", seg.start, seg._end,
						"\\begin{bmatrix}\r a & b \\\\\r c & d \r\\end{bmatrix}", " "))
					yield(Candidate("latex", seg.start, seg._end,
						"\\begin{Bmatrix}\r a & b \\\\\r c & d \r\\end{Bmatrix}", " "))
					matchFlg = true
				elseif keyW == ']' then
				...
				elseif keyW == '{' then
					...
					yield(Candidate("latex", seg.start, seg._end,
						"\\begin{equation}\\begin{cases}\r?&=?\\\\\r?&=?\r\\end{cases}\\end{equation}", " "))
					matchFlg = true
				elseif keyW == '}' then
					for i = 2, 9 do
						yield(Candidate("latex", seg.start, seg._end, "}^{" .. tostring(i) .. "}", " "))
